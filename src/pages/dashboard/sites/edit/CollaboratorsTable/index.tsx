...
const CollaboratorsTable: React.FC<CollaboratorsTableProps> = ({ site }) => {
  ...
  const rows = allUsers.map((rowUser) => {
    ...
    const promotePromptOnFreeTier = !ownerIsPro;
        ...
        <Table.Td align="right">
          {!isRowOwner && (
            <Group gap="xs" justify="flex-end">
              ...
              {canManageRoles && collaborator && role === "collaborator" && (
                <Tooltip
                  label={
                    promotePromptOnFreeTier
                      ? "Upgrade to assign admins"
                      : "Promote to admin"
                  }
                >
                  <ActionIcon
                    ...
                    variant="light"
                    onClick={() => {
                      if (promotePromptOnFreeTier) {
                        notifications.show({
                          color: "yellow",
                          message:
                            "Admin role is a paid feature. Upgrade your plan to assign admins.",
                        });
                        return;
                      }
                      setRole(collaborator.id, "admin");
                    }}
                  >
                    <TbArrowUp size="1em" />
                  </ActionIcon>
              ...
              {canManageRoles && collaborator && role === "admin" && !isSelf && (
