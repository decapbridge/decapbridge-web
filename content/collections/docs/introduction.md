...
This project is not to be confused with _Decap CMS_ which is a distinct open source project that does not offer an hosted service for authentication or user management.
...
This is great for enterprise or advanced users that have the resources to configure an authentication setup tailored to their needs, or that want granular user management in an external system, however, in a lot of cases, users just want something that works out of the gate.

Historically, Netlify offered this (_Decap CMS_ used to be [Netlify CMS](https://www.netlify.com/blog/netlify-cms-to-become-decap-cms/)), however after it stopped its involvement with the project and then [deprecated their Identity service](https://www.netlify.com/changelog/deprecation-netlify-identity/), this left a "git-gateway"-shaped void in the _Decap CMS_ community.

This is how **DecapBridge** became a popular alternative, offering a ready-to-go authentication setup for _Decap CMS_, with **no depencency on Netlify**.
